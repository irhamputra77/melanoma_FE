...
const PatientChatBotPage = () => {
    // id di sini adalah Consultation ID yang 100% selaras dengan endpoint Backend
    const { id } = useParams();
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            setMessages(prev => prev.filter(msg => msg.id !== tempPatientMessage.id));
    ...
    if (errorMessage) {
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-130px)] px-4 text-center">
                ...
                <button onClick={() => navigate(`/patient/messages/${id}`)} className="px-6 py-3 bg-[#0A58CA] text-white font-bold rounded-xl hover:bg-blue-700 transition">Back to Doctor Chat</button>
            </div>
        );
    }

    return (
        <div className="max-w-5xl mx-auto h-[calc(100vh-130px)] flex flex-col">
            ...
            <div className="mb-4 shrink-0 flex items-center justify-between">
                ...
                {/* Tombol Back Langsung Mengarah Kembali Ke Ruang Dokter Yang Sama */}
                <button onClick={() => navigate(`/patient/messages/${id}`)} className="px-4 py-2 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition shadow-sm text-sm">
                    Back to Doctor Chat
                </button>
            </div>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 md:p-6 bg-gray-50/30">
                    <div className="flex justify-center mb-6">
                        <span className="bg-gray-200 text-gray-600 text-[10px] font-bold px-4 py-1.5 rounded-full tracking-widest uppercase shadow-sm border border-gray-300">
                            CONSULTATION ID: #{String(id).substring(0, 8).toUpperCase()}
                        </span>
                    </div>
                    ...
                    {messages.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <p className="text-gray-700 font-bold text-base">Mulai Percakapan AI</p>
                            <p className="text-gray-500 text-sm mt-1 max-w-sm">Pesan yang dikirim di sini juga akan terlihat di riwayat konsultasi dokter Anda.</p>
                        </div>
                    )}
